...
{
	DoctorInfoNavigationView *navigationInfoView = [[NSBundle mainBundle] loadNibNamed:NSStringFromClass([DoctorInfoNavigationView class])
																				 owner:self
																			   options:nil].lastObject;
	[navigationInfoView configureWithDoctor:[MDUserManager sharedManager].currentDoctor];
	[navigationInfoView setUserInteractionEnabled:YES];
	[navigationInfoView addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(openDoctorDetails)]];
	[self.navigationItem setTitleView:navigationInfoView];
}
...
- (MDChatViewController *)chatViewController
{
	...
	_chatViewController.tableView.contentInset = UIEdgeInsetsMake(62.f, 0, 12.f, 0);//TODO: set proper offset
	return _chatViewController;
}
...
- (void)openDoctorDetails
{
	UIStoryboard *storyboard = [UIStoryboard storyboardWithName:@"Main" bundle:nil];
	DoctorInfoViewController *doctorInfo = [storyboard instantiateViewControllerWithIdentifier:NSStringFromClass([DoctorInfoViewController class])];
	[self presentViewController:[[UINavigationController alloc] initWithRootViewController:doctorInfo] animated:YES completion:nil];
}
