...
- (NSString *)localURLForImage:(UIImage *)image withMessage:(NSInteger)messageID
{
	MDWebImageManager *webImageManager = [MDWebImageManager sharedManager];

	//create tmpLocalImageURL
	NSString *tmpImageURL = [NSString stringWithFormat:@"moidoc://images/%@", @(messageID)];
	NSURL *localImageURL = [NSURL URLWithString:tmpImageURL];
	[webImageManager.imageCache storeImage:image forKey:[localImageURL absoluteString]];
	return tmpImageURL;
}
